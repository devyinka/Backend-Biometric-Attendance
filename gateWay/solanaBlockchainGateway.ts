...
try {
  ...
  if (!Array.isArray(parsedKey)) {
    throw new Error("SOLANA_WALLET_PRIVATE_KEY must be a JSON array");
  }
...
const createAttendancePDASeed = (recordId: string): Buffer => {
  const seed = crypto.createHash("sha256").update(recordId, "utf8").digest();

  if (seed.length !== 32) {
    throw new Error(`Invalid PDA seed length: ${seed.length}`);
  }
...
const getAttendancePDA = (recordId: string): [PublicKey, number] => {
  const hashedRecordId = createAttendancePDASeed(recordId);

  console.log("🔑 PDA recordId:", recordId);

  console.log("🔑 PDA hashed seed:", hashedRecordId.toString("hex"));

  console.log("🔑 PDA seed lengths:", {
    attendance: Buffer.byteLength("attendance"),
    recordHash: hashedRecordId.length,
  });

  return PublicKey.findProgramAddressSync(
    [Buffer.from("attendance"), hashedRecordId],
    programId,
  );
};
...
const bufferTo32Array = (buffer: Buffer): number[] => {
  if (buffer.length !== 32) {
    throw new Error(`Hash must be exactly 32 bytes. Got ${buffer.length}`);
  }
...
const createInstructionDiscriminator = (methodName: string): Buffer => {
  return crypto
    .createHash("sha256")
...
const encodeI64 = (value: number | bigint): Buffer => {
...
const serializeVerifyIntegrityArgs = (hashArray: number[]): Buffer => {
...
const decodeString = (data: Buffer, offset: number): [string, number] => {
  if (offset + 4 > data.length) {
    throw new Error("Invalid account data: unable to read string length");
  }
  ...
  if (end > data.length) {
    throw new Error("Invalid account data: string exceeds account size");
  }
...
const decodeAttendanceRecord = (
  ...
  if (data.length < 8) {
    throw new Error("Invalid AttendanceRecord account data");
  }
  ...
  if (offset + 32 > data.length) {
    throw new Error("Invalid AttendanceRecord hash data");
  }
  ...
  if (offset + 8 > data.length) {
    throw new Error("Invalid AttendanceRecord timestamp");
  }
  ...
  if (offset >= data.length) {
    throw new Error("Invalid AttendanceRecord bump");
  }
...
const buildRecordAttendanceInstruction = (
  ...
  const discriminator = createInstructionDiscriminator("record_attendance");

  console.log("========================================");

  console.log("🔑 ANCHOR INSTRUCTION");

  console.log("Instruction: record_attendance");

  console.log("Discriminator:", discriminator.toString("hex"));

  console.log("Expected:", "4f57601819a910c9");
  ...
  const args = serializeRecordAttendanceArgs(
    recordId,
    hashArray,
    timestamp,
    deviceId,
...
const buildVerifyIntegrityInstruction = (
  ...
  const discriminator = createInstructionDiscriminator("verify_integrity");
...
export const SolanaBlockchainGateway = {
  ...
  recordAttendanceHash: async (
    ...
    console.log("⛓️ [Solana] Starting attendance blockchain record...");
    ...
    console.log("Record ID byte length:", Buffer.byteLength(recordId, "utf8"));
    ...
    console.log("Attendance hash:", hash.toString("hex"));

    // --------------------------------------------------------
    // Derive PDA
    // --------------------------------------------------------

    const [recordPDA, bump] = getAttendancePDA(recordId);

    console.log("Attendance PDA:", recordPDA.toBase58());
    ...
    console.log("✅ Solana instruction created");
  ...
  verifyAttendanceHash: async (
    ...
    try {
      ...
      console.log("🔐 [Solana] Verifying attendance...");
      ...
      const [recordPDA] = getAttendancePDA(recordId);
      ...
      if (!accountInfo) {
        console.log("❌ Attendance PDA does not exist");

        return false;
      }

      console.log("✅ Attendance PDA found");
      ...
      const verified = currentHash === expectedHashHex;

      console.log("Stored hash:", currentHash);
  ...
  getAttendanceRecord: async (
    ...
    try {
      ...
      console.log("📦 [Solana] Fetching attendance record...");
      ...
      console.log("✅ Attendance account found");
      ...
      console.log("Decoded blockchain record:", record);
